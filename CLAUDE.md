...
- **Zero hardcoded user paths** — every path goes through `@rh/shared/config`. No absolute home paths (`/Users/<name>`, `C:\Users\<name>`, a personal workspace dir) may appear in `packages/`. Enforced as a regression test (`packages/cli/tests/test-no-identity-refs.js`); spot-check via:
  ```bash
...
- **The identity guard is QUOTE-AGNOSTIC, and was not always.** `packages/cli/tests/test-no-identity-refs.js` matches its split-literal patterns with a `['\"]` character class. It previously hardcoded a literal single quote, which made it blind to identical content written in double quotes — two shipped resolvers carried the maintainer's repo nesting that way and went unreported for as long as the guard existed. The failure was **silent**: the suite was green *on those files*, which reads as "clean" rather than "not checked". If you extend it, make the quote a class — and note the scan is LINE-based, so a literal split across a newline still slips through.
- **Framework-checkout resolution is by DISCOVERY, not a fixed path.** `output/scripts/rh-fw.js`, `output/scripts/rh-daily-validate.js` and `oversight/scripts/rh-config-integrity.js` locate the checkout by scanning candidate roots for a marker (`packages/telemetry/scripts`, or the CLI entry point for daily-validate), bounded to 3 levels / 400 directories because `rh-fw.js` runs on **every hook**. They previously joined a fixed `<org>/<wrapper>/<repo>` segment that only ever resolved on one machine. Precedence: `RH_FRAMEWORK_ROOT` (wins outright, never falls through) > `oversight.json:frameworkRoot` > resolved workspace > `$USERPROFILE/Workspace` > derived OneDrive base. **Machine-specific nesting belongs in `oversight.json`, never in shipped code.** The two `output/` scripts are copied to `~/.claude/scripts/` and must stay dependency-free, so they read `CLAUDE_WORKSPACE` directly rather than importing `@rh/shared/config`. Keep the three in step; `packages/oversight/tests/test-config-integrity.js` now pins the chain (6 cases incl. a non-maintainer nesting and dead-override no-fallthrough).
