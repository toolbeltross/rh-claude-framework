...
- [x] 3.2 Wired into `rh-scribe-table-write.js` (after md + rh_scribe) and `rh-learnings-write.js` `modeCreate`. Flag-gated (`contextDb`), **FAIL-CLOSED**: registers the source via `upsertIngestSource` (content-scanned), then `upsertMemoryArtifact` gated on `source_id`; if source registration fails, NO ungated content write. `logDualWrite` audit per row. `require('./lib/context-db')` is **defensive** (try/catch → no-op fallback) so a missing lib can never break the canonical md write. **Deployment fix:** `context-db.js` + `scribe-db.js` are output-OWNED canonicals that the manifest's blanket `lib` exclusion was skipping — added a `copyFiles {from:'scripts/lib'}` op to output's `install.json` (and taught the manifest engine the `from` option); closes the latent scribe-db hand-deploy gap. Outer-seam verified via tmp-HOME install: both libs land in `~/.claude/scripts/lib/`, deployed writer runs end-to-end (md `wrote:1`, shadows `off`). NOTE: full session UUID not yet captured (only `session_id_short` from the row) — `session_id_full` left null; deferred.
- [x] 3.3 Fixed the verified append-observation gap: `rh-learnings-write.js` `modeAppendObservation` writes one incremental `ctx_memory_observation` (resolves parent artifact by natural key; gated + dup-safe). PR (this).
- [~] 3.4 Privacy gate (steward BLOCK 2) — **built + policy confirmed (2026-06-14); still unwired**. `upsertIngestSource` + pure `classifyDisposition` in `context-db.js`: per-source-file disposition, fail-closed. **Policy (user direction 2026-06-14, `BLOCKLIST_VERSION='2026-06-14.1'`):** (1) hard-exclude private paths → `blocklisted-skipped` (regardless of kind/content); (2) **content scan MANDATORY — never slug-only**: no content scanned ⇒ never `clean`; (3) PII hit (SSN/EIN/long-acct regex) → `review-required`; (4) curated structured kinds (`scribe_md`,`learnings_md`,`oversight_jsonl`,`telemetry_jsonl`) + scanned-clean content → `clean`; big prose logs (`prose_md`,`transcript_jsonl`) and unknown kinds **default-DENY** → `review-required` (manual promotion only). Enforcement is physical: content writers emit `INSERT…SELECT…WHERE EXISTS(source clean/redacted)` when a `source_id` is supplied, so a non-clean source yields zero rows (`{ok:true, gated:true}`). `blocklist_version` recorded per source. Landed in `main` early via PR #84 (concurrent /rh-quit race); kept (inert: flag-off + unwired). **Still open:** `config.privateDirs` is `[]` — populate (e.g. `Personal/`) **before** wiring (3.2/3.3). Tests: classify (private path incl. backslash dir; content-mandatory/no-slug-only; PII kinds; curated→clean; prose/unknown default-deny), guard/builder shape, `_buildIngestSql` shape, RH_TEST_PG end-to-end (clean admits, review-required withholds). Output suite 165/165 incl. PG; zero residue.
- [~] 3.5 Telemetry capture — **lib landed (unwired); wiring is the follow-up**. Pure aggregator `lib/transcript-telemetry.js` (`parseTelemetryLine`/`aggregateSession`/`aggregateSubagentRun`) folds transcript JSONL (verified shape: `message.usage.{input_tokens,output_tokens,cache_read_input_tokens,cache_creation_input_tokens}` + `message.model`; tool_use block count; **cost estimated** via CJS `lib/cost-rates.js` mirror of telemetry's canonical, since transcripts carry no per-line cost). Writers in `context-db.js`: `writeSessionTelemetry` (atomic BEGIN/DELETE×2/INSERT model_usage[]+snapshot/COMMIT — idempotent re-ingest) + `upsertSubagentRun` (ON CONFLICT agent_id). **Not privacy-gated** (token/cost metadata, not content; the ingest slug-blocklist already excludes private projects upstream). Both libs added to output `install.json` so they deploy (verified: 4 canonical libs land in tmp-HOME install). Tests: `test-transcript-telemetry.js` (pure: parse/aggregate/cost-rates/subagent-status), `test-context-db.js` (shape + RH_TEST_PG: idempotent session telemetry with generated `total_tokens`/`tokens_per_min`, subagent upsert with generated `cost_per_min`). Output 177/177 incl. PG; zero residue. **TODO (wiring PR):** extend `rh-transcript-ingest.js` for per-turn `transcript_messages` columns (Part A, contextDb-gated) + per-session `writeSessionTelemetry`/`upsertSubagentRun` calls (Part B). `session_id_full` capture also lands there.
...
| privacy gate (3.4) | output suite 163/163 incl. RH_TEST_PG=1: `classifyDisposition` (private path/backslash dir, PII→review-required, clean, unknown kind), `EXISTS(clean)` guard built into both content writers, `_buildIngestSql` shape, and end-to-end PG proof that a clean source admits the write while a review-required source is withheld (`gated:true`, 0 rows). Zero residue. Policy confirmed 2026-06-14. |
| wiring 3.2/3.3 + deploy | output 167/167 (incl. RH_TEST_PG=1 outer-seam: deployed-style CLI writes a clean cleanup row to `ctx_memory_artifact` via the gate; learnings create→artifact + append→observation), cli 62/62 (manifest `from` op + output-manifest-ships-canonicals assertion), oversight 197/197. **tmp-HOME install**: `context-db.js`+`scribe-db.js` deploy to `~/.claude/scripts/lib/`; deployed `rh-scribe-table-write.js` runs end-to-end with shadows dormant. Defensive require proven (missing lib ⇒ md unaffected). Identity scan clean; zero ctx_ residue. |
