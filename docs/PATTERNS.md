...
**Verification:** The `npm test` suite includes a check (`packages/cli/tests/test-no-identity-refs.js`) that fails the build if any absolute home path (`/Users/<name>`, `C:\Users\<name>`, a personal workspace dir) appears in `packages/`. This runs as part of the framework's own CI to prevent regression.
