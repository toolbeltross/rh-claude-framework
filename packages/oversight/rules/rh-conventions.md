...
keywords: [_index.md, CLAUDE.md, xlwings, openpyxl, absolute path, relative path, session state, MASTER_, CONSOLIDATED_, checkbox tracking, tmp, throwaway artifacts, screenshots]
severity: warn
---
...
- Any file named `MASTER_*.md` or `*_CONSOLIDATED.md` must include a Source Registry section with verification tokens. See `rh-completion-standards.md`.
- Throwaway artifacts (screenshots, scratch outputs, API dumps): route to a gitignored `tmp/`, never repo root. See `rh-throwaway-artifacts.md`.
