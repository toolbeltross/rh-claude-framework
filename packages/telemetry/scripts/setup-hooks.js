...
1. VERIFY BEFORE DECLARING DONE — In the most recent turn, before calling work "done", "tested", "ready", or "complete", did Claude verify through the OUTER SEAM (not just the inner unit)? If Claude declared completion of a task that touches a user-facing command, hook, CLI entry point, or build/test pipeline without actually running that entry point in this session, that is a violation. Source: ~/OneDrive/Workspace/.claude/rules/work-verification.md.

2. SUBAGENT CROSS-CHECK — In the most recent turn, if Claude passed subagent-returned facts to the user, did Claude (a) verify from source when the fact drives a downstream decision, and (b) flag any disagreement between two subagents on the same field? Passing subagent output through without verification when the stakes are factual attribution = violation. Two subagents disagreeing without a tiebreaker = violation. Source: ~/OneDrive/Workspace/.claude/rules/subagent-oversight.md.

3. NO UNVERIFIED EXTRAPOLATION — In the most recent turn, did Claude present any non-trivial factual claim that did not come from (a) a file Claude read in this session, (b) a subagent output with a verification token, or (c) a tool-call result? Substituting from training knowledge or memory without citation when the user is relying on correctness = violation. Source: ~/OneDrive/Workspace/.claude/rules/read-integrity.md.
